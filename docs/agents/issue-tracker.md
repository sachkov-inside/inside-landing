...
Issues and specs for this repository live in `sachkov-inside/inside-landing` GitHub Issues. Run
`gh` inside this clone so repository identity comes from `git remote`.

Product decisions and cross-repository work belong in Workspace; link the Workspace parent
rather than moving the Landing deliverable there. Tracked pull requests use `Closes #<number>`;
